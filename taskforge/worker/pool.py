...
class WorkerPool:
    ...
    async def _monitor_workers(self) -> None:
        ...
        while self.running:
            try:
                ...
                self.logger.error(
                    "Error in worker monitoring", error=str(e), exc_info=True
                )
    ...
    def _update_metrics(self) -> None:
        ...
        for worker_id, stats in self.stats.items():
            metrics.worker_errors.labels(worker_id=worker_id, error_type="total").inc(
                stats["errors"]
            )

            metrics.jobs_processed.labels(worker_id=worker_id).inc(
                stats["jobs_processed"]
            )

        # Update pool metrics
        metrics.active_workers.set(len([w for w in self.workers if w.is_running]))
    ...
    async def scale(self, num_workers: int) -> None:
        ...
        self.logger.info(
            "Scaling worker pool", current=self.num_workers, target=num_workers
        )
        ...
        self.num_workers = num_workers
        self.logger.info(
            "Worker pool scaled", new_size=num_workers, active_workers=len(self.workers)
        )
    ...
    async def get_stats(self) -> Dict:
        ...
        return {
            ...
            "uptime_seconds": (
                ...
                - self.stats[self.workers[0].worker_id]["started_at"]
            ).total_seconds()
            if self.workers
            else 0,
        }
    ...
    async def get_queue_depths(self) -> Dict[str, int]:
        """Get depths of all job queues"""
        depths = {}
        for job_type in self.broker.job_registry:
            depths[job_type] = await self.broker.get_queue_depth(job_type)
        return depths
