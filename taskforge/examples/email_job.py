...
class SendEmailJob(Job[EmailJobInput, EmailJobOutput]):
    ...
    async def validate(self) -> bool:
        ...
        if self.input_data.template_id and not self.input_data.template_id.startswith(
            "tmpl_"
        ):
...
async def main():
    ...
    await orchestrator.register_job(
        SendEmailJob, max_retries=3, timeout_seconds=30, concurrency_limit=50
    )
    ...
    job_id = await orchestrator.submit_job(
        ...
        input_data=EmailJobInput(
            to="user@example.com", subject="Welcome", body="Hello from TaskForge!"
        ),
