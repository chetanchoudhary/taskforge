...
class JobRegistry:
    ...
    def register(
        self, job_class: Type[Job], config: Optional[JobConfig] = None
    ) -> None:
        """
        ...
        logger.info(
            "Registered job type",
            job_type=job_type,
            config=self._configs[job_type].model_dump(),
        )
    ...
    def discover_jobs(cls, package_path: str) -> "JobRegistry":
        ...
        try:
            ...
            logger.error(
                "Failed to import job package", package=package_path, error=str(e)
            )
            return registry
        ...
        for _, name, _ in pkgutil.iter_modules([pkg_path]):
            try:
                ...
                for attr_name in dir(module):
                    ...
                    if isinstance(attr, type) and issubclass(attr, Job) and attr != Job:
                        registry.register(attr)

            except Exception as e:
                logger.error("Failed to load job module", module=name, error=str(e))
